...
void MainWindow::openChatWindow(const QString &contact) {
    // 创建新聊天界面
    QLabel *title2=new QLabel(contact);
    title2->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    ...
    connect(backBtn, &QPushButton::clicked, [=]() {
        contact_stack->setCurrentIndex(0);
    });
    QHBoxLayout *headline=new QHBoxLayout;
    headline->setSpacing(300);
    headline->addWidget(title2);
    headline->addWidget(backBtn);
    // 消息输入区域
    ...
    chatLayout->addLayout(headline,1);
    chatLayout->addWidget(chatDisplay,12);
    chatLayout->addLayout(inputLayout,2);
    ...
    // 加载聊天记录
    Chat::createNewContact(QString::fromStdString(currentUser->username), contact);
    ...
    // 发送消息功能
    connect(sendBtn, &QPushButton::clicked, [contact, chatDisplay, messageInput]() {
    ...
    contact_stack->addWidget(chatpage);
    contact_stack->setCurrentWidget(chatpage);
}
...
void MainWindow::setpage6(QWidget* pg){
    ...
    contact_stack=new QStackedWidget;
    mainLayout->addWidget(contact_stack);

    QWidget *contact_page1=new QWidget;
    contact_stack->addWidget(contact_page1);
    QVBoxLayout *page1_layout=new QVBoxLayout;
    QLabel *title1 = new QLabel("我的联系人");
    title1->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
    ...
    page1_layout->addWidget(contact_scroll, 13);
    contact_page1->setLayout(page1_layout);
